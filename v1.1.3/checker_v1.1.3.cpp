...
int main(){
	...
	for(i=0;i<n;i++){
		...
		while(!compile_thread);//当前进程数量达到上限，等待
	}
	...
	while(true){
		...
		for(i=0;i<n;i++){
			...
			switch(result[i].wait_for(std::chrono::seconds(time_limit))){//等待直到程序结束或超时
		...
		if(Error){
			if(Error&1)write("Runtime Error!\n");
			if(Error&2)write("Time Limit Exceeded!\n");
