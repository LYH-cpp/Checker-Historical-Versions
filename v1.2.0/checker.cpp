...
int main(){
	...
	data_result=std::async(std::launch::async,system,make_data);
	for(i=0;i<n;i++)
	...
	while(true){
		...
		set_color(Light);
		write("Test#",cnt,":\nMaking data");
		for(i=1;data_result.wait_for(std::chrono::seconds(1))!=std::future_status::ready;i=(i+1)&3)
			if(i)write('.');
			else{
				i=1;
				write("\b\b");
		...
		for(i=0;i<n;i++){
			while(run_thread<1);
			run_thread--;
			Wait[i]=std::async(std::launch::async,Judge,i);
		}
		for(i=0;i<n;i++)
			Wait[i].wait();
